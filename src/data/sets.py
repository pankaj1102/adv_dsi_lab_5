...
def load_unscaled_sets(path='../data/processed/', val=False):
    ...
    X_train_unscaled = np.load(f'{path}X_train_unscaled.npy') if os.path.isfile(f'{path}X_train_unscaled.npy') else None
    X_val_unscaled   = np.load(f'{path}X_val_unscaled.npy'  ) if os.path.isfile(f'{path}X_val_unscaled.npy')   else None
    X_test_unscaled  = np.load(f'{path}X_test_unscaled.npy' ) if os.path.isfile(f'{path}X_test_unscaled.npy')  else None
    y_train_unscaled = np.load(f'{path}y_train_unscaled.npy') if os.path.isfile(f'{path}y_train_unscaled.npy') else None
    y_val_unscaled   = np.load(f'{path}y_val_unscaled.npy'  ) if os.path.isfile(f'{path}y_val_unscaled.npy')   else None
    y_test_unscaled  = np.load(f'{path}y_test_unscaled.npy' ) if os.path.isfile(f'{path}y_test_unscaled.npy')  else None

    return X_train_unscaled, y_train_unscaled, X_val_unscaled, y_val_unscaled, X_test_unscaled, y_test_unscaled
...
def save_unscaled_sets(X_train=None, y_train=None, X_val=None, y_val=None, X_test=None, y_test=None, path='../data/processed/'):
    """"Save the unscaled sets locally
    ...
    if X_train is not None:
        np.save(f'{path}X_train_unscaled', X_train)
    if X_val is not None:
          np.save(f'{path}X_val_unscaled',   X_val)
    if X_test is not None:
          np.save(f'{path}X_test_unscaled',  X_test)
    if y_train is not None:
          np.save(f'{path}y_train_unscaled', y_train)
    if y_val is not None:
         np.save(f'{path}y_val_unscaled',   y_val)
    if y_test is not None:
          np.save(f'{path}y_test_unscaled',  y_test)
